...
class MobileNetV3SmallSingleTask(nn.Module):
...
class MobileNetV3LargeSingleTask(nn.Module):
    def __init__(self, num_classes=5, dropout=0.2):
        super().__init__()
...
class MobileNetV2SingleTask(nn.Module):
    def __init__(
        self,
        num_classes=5,
        dropout=0.2,
        ...
        use_att=True,
        att_pool_types=None,
        att_reduction_ratio=16,
        att_no_spatial=False
    ):
        super().__init__()

        if att_pool_types is None:
            att_pool_types = ["avg", "std"]
            ...
            for i in range(n):
                ...
                features.append(
                    InvertedResidualV2(
                        ...
                        expand_ratio=t,
                        use_att=use_att,
                        pool_types=att_pool_types,
                        reduction_ratio=att_reduction_ratio,
                        no_spatial=att_no_spatial
                    )
                )
