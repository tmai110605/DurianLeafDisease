...
def visualize_geo_gradcam(
    ...
    attention_boost : float — hệ số boost vùng attention lên trục Z (mặc định 0.4)
...
def run_pipeline(
    ...
    Pipeline đầy đủ:
      ...
      3. Load classification model + Grad-CAM
      4. Resize & align CAM với depth map
      5. Vẽ Geo-GradCAM 3D
      6. (Tuỳ chọn) lưu overlay 2D và file HTML

    Returns
    -------
    dict chứa prediction results và paths file đã lưu
    """
    ...
    print("[3/5] Loading model & computing Grad-CAM...")
    ...
    cam, cam_class_idx, cam_probs = gradcam(input_tensor, task=task_for_cam, class_idx=None)
    gradcam.remove_hooks()

    # ── 4. Resize CAM → depth size ─────────────────
    print("[4/5] Aligning CAM with depth map...")
    cam_resized = cv2.resize(cam, (new_w, new_h))
    cam_resized = (cam_resized - cam_resized.min()) / (cam_resized.max() - cam_resized.min() + 1e-8)

    # ── 5. Visualize Geo-GradCAM ───────────────────
    print("[5/5] Rendering Geo-GradCAM...")
    html_path = os.path.join(output_dir, f"{base_name}_geo_gradcam_{task_for_cam}.html") \
        if save_3d_html else None
    ...
    # ── (Tuỳ chọn) lưu 2D overlay ─────────────────
    overlay_path = None
    if save_2d_overlay:
        overlay = overlay_cam_on_image(original_rgb, cam)
        overlay_path = os.path.join(output_dir, f"{base_name}_gradcam2d_{task_for_cam}.jpg")
    ...
    return {
        ...
        "overlay_2d_path":      overlay_path,
        "geo_gradcam_html":     html_path,
    }
...
if __name__ == "__main__":
    ...
    result = run_pipeline(
        ...
        task_for_cam="severity",    # "disease" hoặc "severity"
        img_size=224,
        max_size=200,
        attention_boost=0.4,
        output_dir="output",
